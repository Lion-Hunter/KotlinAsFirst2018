...
fun hasAnagrams(words: List<String>): Boolean {
    val wordsMap = mutableMapOf<Set<Char>, Int>()

    words.forEach { if (wordsMap[it.toSet()] == 1) return true else wordsMap[it.toSet()] = 1 }

    return false
}

/* variant, which a little bit slower
{
    val wordsList = words.map { it.toList().sorted() }
    for (i in 0..(wordsList.size - 1)) {
        if (wordsList[i] in wordsList.subList(i + 1, wordsList.size)) return true
    }
    return false
}
*/
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (first in 0 until list.size) {
        second = number - list[first]

        if (second in map)
            return map[second]!! to first
        else map[list[first]] = first
    }
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    val result = mutableListOf(emptySet<String>())
    val pricesList = mutableListOf(0)

    for (available in 1..capacity) {
        var maxPrice = pricesList[available - 1]
        var treasure = emptySet<String>()
        var position = available - 1

        for ((item, value) in mapOfTreasures) {
            if (value.first > available) continue

            val remainder = available - value.first
            var curPrice = pricesList[remainder]

            if (item !in result[remainder]) curPrice += value.second
        ...
        pricesList += maxPrice
        result += result[position] + treasure
    }
